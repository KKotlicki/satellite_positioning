...
function positionInECEF(
	...
	const x: number =
		xk * Math.cos(OmegaK) - yk * Math.cos(inc) * Math.sin(OmegaK)
	const y: number =
		xk * Math.sin(OmegaK) + yk * Math.cos(inc) * Math.cos(OmegaK)
...
function calculateSkyPositions(
	...
	const N =
		a / Math.sqrt(1 - eSquared * Math.sin(latitudeRad) * Math.sin(latitudeRad))
	const xObserver =
		(N + height) * Math.cos(latitudeRad) * Math.cos(longitudeRad)
	const yObserver =
		(N + height) * Math.cos(latitudeRad) * Math.sin(longitudeRad)
	...
	GNSS.forEach((satelliteData, satellite) => {
		...
		satelliteData.forEach(([x, y, z], timeIncrement) => {
			...
			const rTopocentric = Math.sqrt(
				xTopocentric ** 2 + yTopocentric ** 2 + zTopocentric ** 2
			)
...
const useStore = create<Store>((set) => ({
	...
	changeAlmanac: (newAlmanac) =>
		set(({ date }) => ({
			almanac: newAlmanac,
			GNSS: calculateSatellitePositions(newAlmanac, date)
		})),
