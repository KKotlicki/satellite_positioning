...
function parseCoordinate(coordString: string) {
  // Adjusted regex to make the decimal part of the seconds optional
...
function eccentricAnomalyOfOrbit(e: number, Mk: number, E: number = Mk, iteration: number = 0): number {
  // Base case: if the difference between E and Mk is less than a small epsilon, stop the recursion
  // Or if the number of iterations exceeds a certain limit, stop the recursion
  if (Math.abs(E - Mk) < 1e-12) {
    return E;
  }

  // Recursive case: calculate a new estimate for E and call the function again
...
function calculateSatellitePositions(almanac: Map<number, number[]>, date: dayjs.Dayjs) {
  ...
  for (const [satellite, satelliteData] of entries) {
    ...
    if (health === undefined || e === undefined || toa === undefined || inc === undefined || Omega === undefined || sqrtA === undefined || Omega0 === undefined || omega === undefined || M0 === undefined) {
      continue; // Skip this iteration if any required value is undefined
    }

    const a = sqrtA ** 2; // Semi-Major Axis

    if (health === 0) { // Only calculate positions for healthy satellites
...
function calculateSkyPositions(GNSS: SatellitePath, latitude: string, longitude: string, height: number, elevationCutoff: number) {
  ...
  const eSquared = 1 - (b*b)/(a*a); // Square of eccentricity

  // Convert observer's latitude and longitude from degrees to radians
  ...
  // Calculate observer's ECEF position
  ...
  GNSS.forEach((satelliteData, satellite) => {
    ...
    satelliteData.forEach(([x, y, z], timeIncrement) => {
      ...
      // Calculate distance to satellite in topocentric coordinates
      ...
      const azimuth = (Math.atan2(yTopocentric, xTopocentric) * 180 / Math.PI + 360) % 360; // Ensure azimuth is between 0 and 360 degrees


      // Add to positions if elevation is above the cutoff
...
const useStore = create<Store>((set) => ({
  ...
  GNSS: new Map([
    ...
    [3, Array.from({ length: 144 }, (_, index) => {
      const angle = (index / 144) * 2 * Math.PI; // Adjusted denominator to match the length for accurate angle calculation
    ...
    [4, Array.from({ length: 144 }, (_, index) => {
      const angle = (index / 144) * 2 * Math.PI; // Same adjustment as above
