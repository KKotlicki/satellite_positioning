...
import { useDOP, useSelectedTocs, useTime } from "@/services/store"
...
export default function DOPGraph() {
	...
	const shortTimeLabels = selectedTocs.map((toc) =>
		dayjs.utc((toc + 315964800) * 1000).format("HH:mm")
	)
		...
		<Plot
			...
			layout={{
				...
				xaxis: {
					...
					fixedrange: true,
					tickvals: Array.from(
						{ length: Math.ceil(selectedTocs.length / 12) + 1 },
						(_, i) => i * 12
					).filter((val) => val < selectedTocs.length),
