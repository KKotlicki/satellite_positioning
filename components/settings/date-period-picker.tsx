import { useActions, useSelectedTocs } from "@/services/store"
...
export default function DatePeriodPicker() {
	const { changeSelectedTocs } = useActions()
	const selectedTocs = useSelectedTocs()

	const firstToc = selectedTocs[0]
	const lastToc = selectedTocs[selectedTocs.length - 1]
	if (firstToc === undefined || lastToc === undefined)
		throw new Error("First or last TOC is undefined")
	...
	const isValidDate = (date: DateRange<Dayjs>) => {
		return (
			date[0]?.isBefore(date[1]) && date[0]?.isValid() && date[1]?.isValid()
		)
	}
