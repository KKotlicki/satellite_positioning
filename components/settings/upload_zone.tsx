...
export default function UploadZone() {
	...
	const updateStoreData = useCallback(<T extends AstronomyData>(
		content: string,
		fileName: string,
	...
	const initRnxNavigation = useCallback(async () => {
		...
		console.log("laksfm");
		updateStoreData(content, "BRDC00WRD_R_20240650000_01D_GN.rnx", parseRnxNavigation, changeNavigationFile);
	...
	const initRnxObservation = useCallback(async () => {
		...
		const content = await response.text();
		updateStoreData(content, "JOZ200POL_R_20240650000_01D_30S_MO.rnx", parseRnxObservation, changeObservationFile);
