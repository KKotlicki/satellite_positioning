...
export default function VisibilityGraph() {
	...
	const layout: Partial<Layout> = {
		...
		xaxis: {
			...
			fixedrange: true,
			tickvals: Array.from({ length: timeLabels.length }, (_, i) => i).filter(
				(_, index) => index % 12 === 0
			),
		...
		yaxis: {
			...
			fixedrange: true,
			tickvals: Array.from(
				{ length: selectedSatellitesArray.length },
				(_, i) => i
			),
	...
	const handleLegendClick = (event: Readonly<LegendClickEvent>) => {
		...
		const updatedSelectedSatellites = Object.entries(selectedSatellites).reduce(
			(acc, [provider, sats]) => {
				const newSats = Object.keys(sats || {}).reduce(
					(satAcc, prn) => {
