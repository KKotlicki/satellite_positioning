...
export default function SideDrawerSelection(): JSX.Element {
	...
	useEffect(() => {
		const newProviderChecked = satelliteProviders.reduce(
			(acc, curr) => {
				const providerSatellites = selectedSatellites[curr.prefix] || {}
				const isChecked = Object.values(providerSatellites).some(
					(sat) => sat.isSelected
				)
	...
	const setSatelliteSelection = (provider: string, turnOn: boolean) => {
		...
		for (const prn of Array.from(allSatellites)) {
			if (prn.charAt(0) === provider) {
				...
				const healthStatus = satelliteData
					? checkHealthStatus(satelliteData)
					: { health: 1 }
						...
						<FormControlLabel
							...
							control={
								<Checkbox
									...
									onChange={(e) =>
										setSatelliteSelection(provider.prefix, e.target.checked)
									}
								/>
							}
							label={provider.name}
