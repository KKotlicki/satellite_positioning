...
function getSatelliteData(
  ...
  const checkHealthStatus = (satellite: { [toc: number]: { health: number } }): { health: number } => {
...
export default function SatelliteSelection({ provider }: { provider: string }) {
  ...
  return (
    ...
    >
        ...
        {Array.from(satelliteData).map(([prn, health]) => {
          ...
          return (
            <GNSSPaper
              ...
              style={{
                backgroundColor: selectedSatellites[provider]?.[prn]?.isSelected ? theme.palette.divider : theme.palette.background.default,
              }}
            >
      ...
      <Typography variant='body1' color='textSecondary' sx={{ width: '100%', textAlign: 'center', marginTop: '1rem' }}>
