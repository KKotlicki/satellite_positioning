...
const Settings = () => {
  ...
  const isValidLatitude = (value) => {
    ...
    if (/^[-NnSs]?\d{1,2}°\d{1,2}'\d{1,2}(\.\d+)?$|^[-NnSs]?\d+(\.\d+)?$/.test(sanitizedValue)) {
      if (sanitizedValue.includes('°')) {
        const parts = sanitizedValue.match(/([-NnSs]?)(\d{1,2})°(\d{1,2})'(\d{1,2}(\.\d+)?)/);
        if (parts) {
  ...
  const isValidLongitude = (value) => {
    ...
    if (/^[-EeWw]?\d{1,3}°\d{1,2}'\d{1,2}(\.\d+)?$|^[-EeWw]?\d{1,3}(\.\d+)?$/.test(sanitizedValue)) {
      if (sanitizedValue.includes('°')) {
        const parts = sanitizedValue.match(/([-EeWw]?)(\d{1,3})°(\d{1,2})'(\d{1,2}(\.\d+)?)/);
        if (parts) {
          const degrees = parseFloat(parts[2]);
          const minutes = parseFloat(parts[3]) / 60;
          const seconds = parseFloat(parts[4]) / 3600;
  ...
  const parseLatitude = (value) => {
    const sanitizedValue = value.replace(/\s+/g, '');
    if (/^[-NnSs]?\s?\d{1,2}°\s?\d{1,2}'\s?\d{1,2}(\.\d+)?$/.test(sanitizedValue)) {
      if (sanitizedValue.includes('°')) {
        const parts = sanitizedValue.match(/([-NnSs]?)\s?(\d{1,2})°\s?(\d{1,2})'\s?(\d{1,2}(\.\d+)?)?/);
        if (parts) {
          const degrees = parseFloat(parts[2]);
    ...
    } else if (/^[-NnSs]?\s?\d+(\.\d+)?$/.test(sanitizedValue) || /^[-NnSs]?\s?\d{1,2}$/.test(sanitizedValue)) {
